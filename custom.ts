...
//% weight=100 color=#7B61FF icon="\uf084" block="문열고 닫는 코드 최적화"
namespace DoorControl {
    let position: Position = null

    //% blockId=OpenDoor block="$n째 문 열기"
    export function OpenDoor(n: number){
        position = player.position()
        if (position.getValue(Axis.X) == 16 - n * 2 && (position.getValue(Axis.Y) == 6 && (position.getValue(Axis.Z) == -88 || position.getValue(Axis.Z) == -89))) {
            if (n > 1 || n < 15) {
                player.execute(
                "/setblock" + " " + world(14 + -2 * (n - 1), 3, -87) + " " + "smooth_quartz"
                )
            } else {
                player.execute(
    ...
    //% blockId=CloseDoor block="$n째 문 닫기"
    export function CloseDoor(n: number){
        position = player.position()
        if (position.getValue(Axis.X) == 16 - n * 2 && (position.getValue(Axis.Y) == 6 && (position.getValue(Axis.Z) == -88 || position.getValue(Axis.Z) == -89))) {
            if (n > 1 || n < 15) {
                player.execute(
                "/setblock" + " " + world(14 + -2 * (n - 1), 3, -87) + " " + "redstone_block"
                )
            } else {
                player.execute(
                "tellraw "+"@s "+"{\"rawtext\":"+"[{\"translate\":"+"\"say.wrong_local\",\"with\":"+"{\"rawtext\":"+"[{\"text\":"+"\"\n\""+"}]}}]}"
